...
from resnet_simclr_omni import ResNetSimCLR
...
#experiment_root = '/home/misc/RnD_project/ResNet-18-fewshot-prototypical-ntw/output_5/'
model_name = "resnet"
num_classes = 128
...
def init_dataset(opt):
    ...
    if opt.dataset == 'omniglot':
        ...
        test_dataset = OmniglotDataset(mode='test5')
    ...
    #trainval_sampler = PrototypicalBatchSampler(labels=trainval_dataset.y, classes_per_it=opt.classes_per_it_tr,                                num_samples=opt.num_support_tr +opt.num_query_tr, iterations=opt.iterations)
    ...
    #trainval_dataloader = torch.utils.data.DataLoader(trainval_dataset, batch_sampler=trainval_sampler)
...
def init_protonet(opt):
    ...
    checkpoint = torch.load('checkpoint_0050.pth.tar', map_location='cpu')
...
def train(opt, tr_dataloader, model, optim, lr_scheduler, val_dataloader=None):
    ...
    for epoch in range(opt.epochs):
        ...
        for batch in tqdm(tr_iter):
            ...
            #print("====================================================================")
            #print(x.shape,y.shape,model_output.shape)
            l, acc = loss(model_output, target=y, n_support=opt.num_support_tr)
            l.backward()
            optim.step()
            l = l.squeeze().detach().cpu().numpy()
        ...
        if avg_acc >= best_acc:
            ...
            torch.save(model.state_dict(),'/home/misc/RnD_project/Third_expt/128_imagenet_{}_{}_{}_5.pth'.format(opt.classes_per_it_tr,opt.num_support_tr,opt.iterations))
