...
def info_nce_loss(features, batch_size, n_views=2):
        temperature = 0.07
        ...
        #mask = torch.eye(600, dtype=torch.bool).to(device)
        labels = labels[~mask].view(labels.shape[0], -1)
        #print("labels, Sim and mask: ",labels.shape[0],similarity_matrix.shape,mask.shape)
...
def prototypical_loss(input, prototypes, target, n_support, inner_loop=False):
    ...
    if inner_loop:
        ...
        oq_idxs_0 = torch.stack(list(map(lambda c: torch.nonzero(cputargs.eq(int(c)))[n_support:], classes))).view(-1)
        #print(torch.nonzero(cputargs.eq(int(1)))[n_support:].shape)
